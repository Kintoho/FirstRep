...
public class Array {
    ...
    public static int[] twentySixthArray(int[] numbers) {
        int[] array = new int[numbers.length/2 + numbers.length % 2];
        int k = 0;
        if (numbers.length % 2 == 1) {
            array[numbers.length / 2] = numbers[numbers.length - 1];
            for (int i = 0; i < numbers.length-2; i+=2) {
                array[k] = numbers[i] + numbers[i + 1];
                k++;
            }
        }
        else {
